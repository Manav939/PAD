...
class VAE(nn.Module): 
    def __init__(self,in_shape,z_dim, *args, **kwargs):
        ...
        self.in_shape = in_shape
        self.in_dim = in_shape[-1] * in_shape[-2]
        self.z_dim = z_dim
        self.non_linear = nn.ReLU()
        self.p = 0.1
        self.ldj = 0
        self.encoder = nn.ModuleList([nn.Linear(self.in_dim,self.dim_list[0]),self.non_linear])
        for i in range(len(self.dim_list)-1):
            self.encoder.append(nn.Linear(self.dim_list[i], self.dim_list[i+1]))
            self.encoder.append(nn.Dropout(self.p))
            self.encoder.append(nn.BatchNorm1d(self.dim_list[i+1]))
            self.encoder.append(self.non_linear)
        self.encoder.append(nn.Linear(self.dim_list[-1],z_dim*2))

        self.decoder = nn.ModuleList([nn.Linear(z_dim,self.dim_list[-1])])
        for i in range(len(self.dim_list)-1):
            self.decoder.append(self.non_linear)
            self.decoder.append(nn.BatchNorm1d(self.dim_list[-1-i]))
            self.decoder.append(nn.Dropout(self.p))
            self.decoder.append(nn.Linear(self.dim_list[-1-i],self.dim_list[-2-i]))
        self.decoder.append(nn.Sequential(self.non_linear,nn.Linear(self.dim_list[0],self.in_dim)))
    ...
    def forward(self,x):
        ...
        return x , mean , torch.exp(log_var*0.5) , self.ldj , z , z# Reconstructed Input & latent vector x
